...
test('endpoint interno exige HMAC, timestamp e nonce', () => {
  ...
  const hmac = read('portal_hmac.js');
  ...
  assert.match(hmac, /timingSafeEqual/);
  assert.match(hmac, /nonce-repetido/);
});
...
test('notificações de segurança usam tipos fechados e não aceitam HTML arbitrário', () => {
  ...
  for (const type of ['FIRST_ACCESS', 'PASSWORD_RESET', 'PASSWORD_CREATED', 'PASSWORD_CHANGED']) {
...
test('links de ativação/reset precisam permanecer no domínio HTTPS do Portal', () => {
  const mod = read('security_validation.js');
